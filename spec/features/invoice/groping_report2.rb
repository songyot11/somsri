describe 'Grouping Report Type 2', js: true do
  ...
  let(:students) do
    [
      Student.make!(student_number: 1, first_name: 'สมศรี', last_name: 'ศรีสุข', grade_id: grades[0].id ),
      Student.make!(student_number: 2, first_name: 'มั่งมี', last_name: 'ศรีสุข', nickname: 'รวย', classroom_number: 13, grade_id: grades[0].id),
  ...
  describe 'Grouping report' do
    it 'should display report page' do
      visit '/somsri_invoice#/grouping_report2'
      sleep(1)
      expect(page).to have_content("#{yesterday_str} 0.00 1,000.00 10,000.00 11,000.00")
      expect(page).to have_content("#{today_str} 200,000.00 2,000.00 10,000.00 212,000.00")
      expect(page).to have_content("รวมทั้งหมด 200,000.00 3,000.00 20,000.00 223,000.00")
    end
    ...
    it 'should display report page only ค่าเสื้อ' do
      ...
      sleep(1)
      expect(page).to have_content("#{yesterday_str} 1,000.00 10,000.00 11,000.00")
      expect(page).to have_content("#{today_str} 2,000.00 210,000.00 212,000.00")
      expect(page).to have_content("รวมทั้งหมด 3,000.00 220,000.00 223,000.00")
    end

    it 'should display report per student by click link' do
      visit '/somsri_invoice#/grouping_report2'
      sleep(1)
      first('td:nth-child(2) a').click
      sleep(1)
      expect(page).to have_content("มั่งมี ศรีสุข (รวย) Preschool 0.00 1,000.00 10,000.00 11,000.00")
      expect(page).to have_content(" รวมทั้งหมด 0.00 1,000.00 10,000.00 11,000.00")
    end
    ...
    it 'should display report per student by select same date' do
      ...
      sleep(1)
      expect(page).to have_content("สมศรี ศรีสุข Preschool 100,000.00 2,000.00 0.00 102,000.00")
      expect(page).to have_content("มั่งมี ศรีสุข (รวย) Preschool 100,000.00 0.00 10,000.00 110,000.00")
      expect(page).to have_content("รวมทั้งหมด 200,000.00 2,000.00 10,000.00 212,000.00")
    end
    ...
    it 'should display report per student by select same date and display only ค่าเสื้อ' do
      ...
      find('.dropdown-menu label', text: 'ค่าเทอม').click
      sleep(1)
      expect(page).to have_content("สมศรี ศรีสุข Preschool 2,000.00 100,000.00 102,000.00")
      expect(page).to have_content("มั่งมี ศรีสุข (รวย) Preschool 0.00 110,000.00 110,000.00")
      expect(page).to have_content("รวมทั้งหมด 2,000.00 210,000.00 212,000.00")
    end
    ...
    it 'should qry from pass to present' do
      ...
      expect(page).to have_content("#{yesterday_str} 1,000.00 10,000.00 11,000.00")
      expect(page).to have_content("#{today_str} 200,000.00 2,000.00 10,000.00 212,000.00")
      expect(page).to have_content("รวมทั้งหมดi 200,000.00 3,000.00 20,000.00 223,000.00")
    end
  end
